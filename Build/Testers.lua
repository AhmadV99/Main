...
local Status = Window:MakeTab({"Status", "rbxassetid://17829819551"})
local Status1 = Status:AddSection({"World"})
local Status2 = Status:AddSection({"Race"})
local Status3 = Status:AddSection({"Level"})
local Status4 = Status:AddSection({"Fruit"})
local Status5 = Status:AddSection({"Bounty"})
Status:AddSection({"- [ Sword Legendary ] -"})
local Status6 = Status:AddSection({"Shisui"})
task.spawn(function()
    while task.wait() do
        pcall(function()
            Status1:Set("World: ".. CheckStatus("World"))
            Status2:Set("Race: ".. CheckStatus("Race"))
            Status3:Set("Level: ".. CheckStatus("Level"))
            Status5:Set("Bounty: ".. CheckStatus("Bounty"))
            Status6:Set("Shisui: ".. CheckStatus("Shisui"))
        end)
    end
end)
if World2 or World3 then
